...
func (b *Builder) Set(key string, value interface{}) {
	b.jsonMap = setInternal(key, b.jsonMap, value)
}

func setInternal(key string, p map[string]interface{}, value interface{}) map[string]interface{} {
	...
	if len(keys) == 1 {
		...
		if _, ok := p[keys[0]]; ok {
			if reflect.TypeOf(p[keys[0]]) == reflect.TypeOf(p) {
				p[keys[0]] = setInternal(strings.Join(keys[1:], "."), p[keys[0]].(map[string]interface{}), value)
				return p
			} else {
